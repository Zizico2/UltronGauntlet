...
const (
	mandatory       examsType = iota
	groups          examsType = iota
	undiferentiated examsType = iota
)

type examsParsed struct {
	description string
	exams       []exam
	examGroups  [][]exam
}

type examParsed struct {
	code uint
	name string
}

type courseInfoParsed struct {
	courseName      string
	institutionName string
	courseCode      string // should be uint
	institutionCode string // should be uint
	degree          string
	ects            uint
	cnaef           string
	duration        string // parse this
	ctype           string
	contest         string
	examsInfo       examsInfo
}
...
func main() {
	...
	c.OnHTML("div#caixa-orange", func(e *colly.HTMLElement) {
		...
		course.examsInfo.etype = mandatory
		examsStage := -1
			...
			if examsStage > 0 {
				...
				if strings.Contains(text, ":") {
					...
					if strings.Contains(text, "conjuntos") {
						course.examsInfo.etype = groups
						groupExams = true
					}
				} else {
					course.examsInfo.etype = undiferentiated
